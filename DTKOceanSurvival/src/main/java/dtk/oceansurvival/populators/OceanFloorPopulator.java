...
public class OceanFloorPopulator extends BlockPopulator {
    ...
    public void populate(WorldInfo worldInfo, Random random, int chunkX, int chunkZ, LimitedRegion limitedRegion) {
        ...
        for(int iteration = 0; iteration < random.nextInt(48) + 16; iteration++) {
            ...
            int surfaceHeight = limitedRegion.getHighestBlockYAt(x,z)-1;
            for (int y = surfaceHeight; y > depth; y--) {
                if (limitedRegion.isInRegion(x, y, z)) {
                    ...
                    if (block.isOccluding() && y < surfaceHeight-2) {
